...
Describe 'Resolve-ArcGisPythonPath' {
    ...
    It 'ignores a custom/cloned env (e.g. the default "gis-env" env) and never resolves to it' {
        # Isolation regression test: this tool must stay standalone from dcpgis and only
        # ever call the machine's default arcgispro-py3 env - even when a custom clone is
        # the only interpreter actually present, it should NOT be picked up as a fallback.
        $env:ProgramW6432 = 'C:\Program Files'
        $env:ProgramFiles = 'C:\Program Files (x86)'
        Mock Test-Path {
            param($Path)
            $Path -eq 'C:\Program Files\ArcGIS\Pro\bin\Python\envs\gis-env\python.exe'
        }
