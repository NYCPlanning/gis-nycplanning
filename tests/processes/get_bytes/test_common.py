...
def test_get_zip_central_directory_returns_sizes_and_total(monkeypatch):
    # The whole point of preferring this over get_zip_namelist: per-member sizes and the
    # zip's total size both come back from the one request, no extra round trips.
    zip_bytes = make_zip_bytes(
        ["a.shp", "data.csv"], content={"data.csv": b"col1,col2\n1,2\n"}
    )
    routes = {URL: ranged_zip_response(zip_bytes, total_size=987654)}
    monkeypatch.setattr(requests.Session, "get", mock_session_call(routes))

    result = get_zip_central_directory(URL, make_session())
    assert result is not None
    infolist, total = result

    assert [info.filename for info in infolist] == ["a.shp", "data.csv"]
    assert {info.filename: info.file_size for info in infolist} == {
        "a.shp": 0,
        "data.csv": 14,
    }
    assert total == 987654
...
def test_get_zip_central_directory_total_is_none_without_content_range(monkeypatch):
    # A 206 with no usable Content-Range is the one case where size genuinely isn't knowable
    # without another request - report that honestly rather than guessing from the tail.
    routes = {URL: MockResponse(206, make_zip_bytes(["a.shp"]))}
    monkeypatch.setattr(requests.Session, "get", mock_session_call(routes))
...
def test_get_zip_central_directory_total_from_full_response_length(monkeypatch):
    # Status 200 means Range was ignored and the whole file arrived, so its length IS the
    # total - no Content-Range header needed.
    zip_bytes = make_zip_bytes(["a.shp"])
    routes = {URL: MockResponse(200, zip_bytes)}
    monkeypatch.setattr(requests.Session, "get", mock_session_call(routes))
