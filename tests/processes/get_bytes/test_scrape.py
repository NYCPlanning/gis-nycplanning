"""Tests for scrape.py - fully offline (see conftest.block_network)."""
...
def test_page_slug_from_url():
    assert (
        scrape.page_slug_from_url(
            "https://www.nyc.gov/content/planning/pages/resources/datasets/mappluto-pluto-change"
        )
        == "mappluto-pluto-change"
    )


def test_page_slug_from_url_ignores_trailing_slash_and_query():
    assert scrape.page_slug_from_url("https://x/a/b/my-page/") == "my-page"
    assert scrape.page_slug_from_url("https://x/a/b/my-page?y=1") == "my-page"


def test_normalize_dataset_name_strips_tm_symbol_and_suffix():
    assert scrape.normalize_dataset_name("MapPLUTO™ - Shapefile") == "mappluto"


def test_normalize_dataset_name_multi_word():
    assert scrape.normalize_dataset_name("PLUTO Change File") == "pluto_change_file"


def test_reference_doc_suffix():
    assert scrape.reference_doc_suffix("View Data Dictionary") == "datadictionary"
    assert scrape.reference_doc_suffix("View Read Me") == "readme"


def test_infer_type_from_label():
    assert scrape.infer_type_from_label("File Geodatabase") == "fgdb"
    assert scrape.infer_type_from_label("Shapefile (SHP)") == "shp"
    assert scrape.infer_type_from_label("Data (CSV)") == "csv"
    assert scrape.infer_type_from_label("Something else") is None


def test_infer_type_from_url():
    assert scrape.infer_type_from_url("https://x/nyc_mappluto_26v2_fgdb.zip") == "fgdb"
    assert scrape.infer_type_from_url("https://x/nyc_mappluto_26v2_shp.zip") == "shp"
    assert scrape.infer_type_from_url("https://x/mappluto_18v1.zip") is None


def test_strip_cache_buster():
    assert scrape.strip_cache_buster("https://x/file.zip?r=1") == "https://x/file.zip"
    assert scrape.strip_cache_buster("https://x/file.zip") == "https://x/file.zip"
...
def test_infer_type_pdf_shortcut_ignores_label(monkeypatch):
    ...
    assert (
        scrape.infer_type("anything", "https://x/readme.pdf", requests.Session())
        == "pdf"
    )
...
def test_infer_type_from_zip_contents(monkeypatch):
    ...
    assert scrape.infer_type_from_zip_contents(url, requests.Session()) == "fgdb"
    ...
    assert scrape.infer_type_from_zip_contents(url2, requests.Session()) == "shp"
    ...
    assert scrape.infer_type_from_zip_contents(url3, requests.Session()) == "txt"
    ...
    assert scrape.infer_type_from_zip_contents(url4, requests.Session()) == "unknown"
...
def test_infer_type_from_zip_contents_unlistable_zip_is_unknown(monkeypatch):
    ...
    assert scrape.infer_type_from_zip_contents(url, requests.Session()) == "unknown"
...
def test_assign_identifiers_no_collision():
    ...
    scrape.assign_identifiers(rows)
...
def test_assign_identifiers_collision_gets_stable_distinct_suffixes():
    ...
    scrape.assign_identifiers(rows2)
...
def test_parse_recent_release(mocked_responses_path):
    ...
    rows = scrape.parse_recent_release(html_fragment, requests.Session())
...
def test_parse_archive(monkeypatch, mocked_responses_path):
    ...
    rows = scrape.parse_archive(entries, requests.Session())
    by_url = {row["url"]: row for row in rows}
...
def test_fetch_page_rows_derives_both_endpoints_from_page_slug(
    monkeypatch, mocked_responses_path
):
    # Both endpoint URLs are templated off the page slug rather than hardcoded, which is what
    # lets this tool point at another DCP dataset page of the same shape without a code change.
    html_fragment = (mocked_responses_path / "recent_release_fragment.html").read_text()
    routes = {
        "https://apps.nyc.gov/content-api/v1/content/planning/resources/datasets/some-page": MockResponse(
            200, json.dumps({"description": html_fragment}).encode()
        ),
        "https://www.nyc.gov/assets/planning/json/content/resources/dataset-archives/some-page.json": MockResponse(
            200, b"[]"
        ),
    }
    monkeypatch.setattr(requests.Session, "get", mock_session_call(routes))

    rows = scrape.fetch_page_rows("some-page", requests.Session())

    assert rows, "expected the recent-release fixture to yield rows"
    assert all("identifier" in row for row in rows)
