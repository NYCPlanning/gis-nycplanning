...
A GitHub Actions workflow (`.github/workflows/lint.yml`) runs `ruff`, `mypy`, and `PSScriptAnalyzer` against every pull request, using a hosted `ubuntu-latest` runner. It installs dev packages via conda from [`conda-requirements-dev.txt`](https://github.com/NYCPlanning/gis-nycplanning/blob/main/deployment/conda-requirements-dev.txt), the same file the local deploy script uses, so CI and local devs see identical versions. The two requirements files are kept separate specifically so this job can install only `conda-requirements-dev.txt` and skip the larger `geopandas`/`fiona`/`numpy` packages in `conda-requirements.txt`. This is for CI performance reasons, not because of any consumer-vs-contributor distinction (this package isn't distributed). 
