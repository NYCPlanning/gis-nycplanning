...
def main():
    ...
    SOURCE_CONNECTION_FILE_NAME: str = settings_product["source_connection_file"][
        "name"
    ]
    SOURCE_SCHEMA: str = settings_product["source_connection_file"]["schema"]
    DESTINATION_CONNECTION_FILE_NAME: str = settings_product[
        "destination_connection_file"
    ]["name"]
    ...
    OPEN_DATA_STAGING_PATH: Path = Path(
        settings_global["open_data_staging_path"]
    ).absolute()
    CONNECTION_FILE_PATH: Path = Path(
        settings_global["connection_file_path"]
    ).absolute()
    CYCLE_DATE: str = date_logic.calc_open_data_cycle_month(
        settings_global["open_data_cycle_date"]
    )

    # Define secondary constants
    SOURCE_SDE_PATH: Path = Path(CONNECTION_FILE_PATH / SOURCE_CONNECTION_FILE_NAME)
    source_middle = SOURCE_CONNECTION_FILE_NAME.removeprefix("sde@GIS").removesuffix(
        ".sde"
    )
    SOURCE_SDE_PREFIX: str = f"GIS{source_middle}.{SOURCE_SCHEMA}."
    SOURCE_SDE_DZM_PATH: Path = Path(
        SOURCE_SDE_PATH / f"{SOURCE_SDE_PREFIX}Digital_Zoning_Map"
    )
    OPEN_DATA_STAGING_YEAR_PATH: Path = Path(
        OPEN_DATA_STAGING_PATH / "zoning" / CYCLE_DATE[:4]
    )
    ...
    COUNCIL_DATE = date_logic.get_latest_date_from_field(
        feature_class_path=str(
            SOURCE_SDE_DZM_PATH
            / f"{SOURCE_SDE_PREFIX}{ZONING_CONVENTIONS['nyzma']['trd_fc_name']}"
        ),
        date_field="EFFECTIVE",
        override_config_value=settings_global[
            "city_council_date"
        ],  # defaults to None if blank in config file
    )
    ...
    logging.debug(
        f"DESTINATION_CONNECTION_FILE_NAME: {DESTINATION_CONNECTION_FILE_NAME}"
    )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        logging.debug(
            f"The following GDBs referenced in constants.py will be created: {GDB_NAMES}"
        )
        ...
        for gdb_name in GDB_NAMES:
            arcpy.management.CreateFileGDB(
                out_folder_path=os.path.join(temp_cycle_dir, "gdb"), out_name=gdb_name
            )
            ...
            if feature_info["desired_fields"]:
                zoning_utils.drop_fields_from_fc(
                    workspace=os.path.join(
                        temp_cycle_dir, "gdb", feature_info["gdb_name"]
                    ),
        ...
        zoning_utils.dissolve_in_place(
            workspace=os.path.join(
                temp_cycle_dir, "gdb", ZONING_CONVENTIONS["nysp"]["gdb_name"]
            ),
        ...
        for _, feature_info in ZONING_CONVENTIONS.items():
            ...
            shp_path = (
                temp_cycle_dir / "shp" / f"{feature_info['public_output_name']}.shp"
            )
            ...
            zoning_utils.import_and_clean_feature_metadata(
                in_feature=shp_path, md_template_file=updated_xml_path
            )
