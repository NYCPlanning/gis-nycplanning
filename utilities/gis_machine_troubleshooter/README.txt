...
2. If you have an ArcGIS Pro project to include, you'll be prompted for the
   path to its .aprx file. Use quotes to ensure the path is read correctly 
   (e.g. "C:\project.aprx", not C:\project.aprx). Leave this blank to skip 
   GIS-level checks.
...
- internal/ holds the implementation: troubleshooter_functions.ps1
  (PowerShell) and collect_gis_info.py (Python, requires the default ArcGIS Pro
  conda env).
- No config file, and no deploy script - copy the
