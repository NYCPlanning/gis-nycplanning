...
**Files:**
- `run_troubleshooter.ps1` - the entry point. Collects machine-level info directly (registry, built-in cmdlets), then shells out to `collect_gis_info.py` for anything that requires `arcpy`, and assembles the final report.
...
- Report generation prefers Excel COM automation (a true multi-tab `.xlsx`, no extra installs required) and falls back to plain `.csv` files if Excel isn't available - see `Write-SheetSections` / `Export-SectionsToCsv` in `run_troubleshooter.ps1`, which both consume the same section data so the two output paths can't drift apart.
...
**Deploying:** there's currently no deploy script - manually copy the `gis_machine_troubleshooter/` directory (containing three files) to the network drive location after merging a change. This can be improved upon over time as needed.
